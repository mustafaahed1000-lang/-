    ...
    const generatePlan = async () => {
        ...
        try {
            ...
            if (targetDocs.length > 0) {
                // Extract chunks and take a large sample to give the AI good context
                const allChunks = targetDocs.flatMap(doc => doc.chunks).map((c: any) => c.text ? c.text : c);
                const sampleChunks = allChunks.slice(0, 20);
                contextStr = sampleChunks.join('\n---\n').substring(0, 20000);
            }

            const prompt = `أنت خبير في التخطيط الأكاديمي وطرق التعليم الحديثة لجامعة القدس المفتوحة.
الطالب يريد دراسة المواد/الملفات التالية: "${formData.subjects.join('، ')}".
لديه "${formData.hoursPerDay}" ساعات يومياً للدراسة.
يفضل أسلوب الدراسة: "${formData.methodology}".

المحتوى المستخرج من هذه الملفات (كدليل لك):
${contextStr ? contextStr : "لا يوجد محتوى نصي مستخرج."}

أوامر عسكرية صارمة جداً:
1. إذا اكتشفت من المحتوى المرفق أعلاه أنه عبارة عن "أسئلة امتحان" أو "اختبر نفسي" أو "أسئلة سنوات سابقة"، **يجب عليك أولاً وقبل كل شيء حل هذه الأسئلة بالكامل بشكل مباشر** وتوفير الإجابة النموذجية مع الشرح، ثم بعد ذلك تضع خطة دراسية لكيفية مراجعتها. إياك أن تؤلف عناوين وهمية لدروس وتتجاهل حل الأسئلة!
2. إذا كان المحتوى المرفق هو كتاب أو منهج نظري، قم بتصميم جدول دراسي عملي وحقيقي لختم هذا المنهج.
3. الجدول يجب أن يكون مقسّم بالساعات والدقائق لهذا الطالب بناءً على الأسلوب الذي اختاره.
4. لا تعطني كلاماً عاماً. التدريب العملي وحل الأسئلة هو الأهم.
5. استخدم Markdown لإنشاء جداول (Tables) وأبرز الأوقات والعناوين بخط عريض، وضع إيموجيات ملائمة 🚀📚.`;
